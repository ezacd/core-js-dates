...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const different = secondDate.getTime() - firstDate.getTime();
  return different / (1000 * 60 * 60 * 24) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    start.getTime() <= thisDate.getTime() && thisDate.getTime() <= end.getTime()
  );
}
...
function formatDate(date) {
  ...
  const time = thisDate.toLocaleTimeString('en-US', {
    hour12: true,
    timeZone: 'UTC',
  });
...
function getWeekNumberByDate(date) {
  const thisDate = new Date(date);
  const firstJan = new Date(thisDate.getFullYear(), 0, 1);
  const days = (thisDate - firstJan) / (24 * 60 * 60 * 1000);
